...
class TD3():
    ...
    def parameter_explore(self):
        self.expl_policy_actor = copy.deepcopy(self.actor)                                           
        cur_weights = self.actor.get_weights()
        perturbed_weights = []
        for cur_weight in cur_weights:
            noise = np.random.normal(0, 0.1, size=cur_weight.shape)
            perturbed_weights.append(noise + cur_weight)
        self.expl_policy_actor.set_weights(perturbed_weights)
