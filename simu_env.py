...
class Env(object):
	def __init__(self, display, fake_env, field, craft_state,
                    min_dist,
                    noise_sigma,
                    in_bounds,
                    goal_bounds,
		...
		self.init_craft_state = copy.deepcopy(craft_state)
		self.craft_state = copy.deepcopy(self.init_craft_state)
	...
	def reset(self):
		...
		self.field.random_init() # randomize the init position of asteriods
		cx,cy,_ = self.craft_state.position
		obstacle_id, obstacle_pos, _ = self.find_nearest_obstacle(cx,cy)
		state = [cx, cy, self.craft_state.v_x, self.craft_state.v_y]
	...
	def find_nearest_obstacle(self, cx, cy, unsafe_obstacle_ids = []):
		astlocs = self.field.asteroid_locations(self.cur_step, cx, cy, self.min_dist * 5)
		nearest_obstacle = None
		nearest_obstacle_id = -1
		nearest_obstacle_dist = np.float("inf")    
		collisions = ()
		for i,x,y in astlocs:
			self.display.asteroid_at_loc(i,x,y)
			if (i in unsafe_obstacle_ids):
				self.display.asteroid_set_color(i, 'blue')
	...
	def save_env(self):
		...
		self.craft_state_copy = copy.deepcopy(self.craft_state)
		self.field_copy = copy.deepcopy(self.field)
		return

	def read_env(self):
		self.cur_step = self.cur_step_copy
		self.craft_state = self.craft_state_copy
	...
	def step(self, action, is_safe = False, unsafe_obstacle_ids = []):
		...
		'''
		self.cur_step += 1
		self.craft_state = self.craft_state.steer( action[0], action[1] )
		cx,cy,ch = self.craft_state.position
		self.display.craft_at_loc( cx, cy, ch, is_safe)
		nearest_obstacle_id, nearest_obstacle, collisions = self.find_nearest_obstacle(cx, cy, unsafe_obstacle_ids)

		next_craft_state = [cx, cy, self.craft_state.v_x, self.craft_state.v_y]
		relative_pos = [cx - nearest_obstacle[0], cy - nearest_obstacle[1]]
		next_state = next_craft_state + relative_pos
	...
	def suboptimal_control(self):
		...
		for id in unsafe_obstacle_ids:
			obstacle_path = []
			obstacle = self.field.asteroids[id]
		...
		for ax in possible_ax:
			for ay in possible_ay:
				vehicle_path = []
				vx = min(self.craft_state.v_x + ax, self.craft_state.max_speed)
				vy = min(self.craft_state.v_y + ay, self.craft_state.max_speed)
				for i in range(1, self.forecast_steps+1):
					vehicle_path.append([self.craft_state.x+vx*i, self.craft_state.y+vy*i])
				...
				if (self.fake_env.model.reward_mse < 1e-3):
					robot_state = np.array([self.craft_state.x, self.craft_state.y, self.craft_state.v_x, self.craft_state.v_y])
	...
	def find_unsafe_obstacles(self, min_dist):
		cx, cy, _ = self.craft_state.position
		unsafe_obstacles = self.field.unsafe_asteroid_locations(self.cur_step, cx, cy, min_dist)
